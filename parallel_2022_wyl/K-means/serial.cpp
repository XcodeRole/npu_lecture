...
//返回最小误差SSE
double getSSE(const double *const* matrix,const int row_dim,const int col_dim,const double * const*center,const set<int> cluster[K]){
    double sse=0;
    for (int i=0;i<K;i++){
...
void k_means(const double *const* matrix,const int row_dim,const int col_dim){
    ...
    double pre_sse=getSSE(matrix,row_dim,col_dim,center,cluster);
    double sse;
    double delta_sse;
    do{
